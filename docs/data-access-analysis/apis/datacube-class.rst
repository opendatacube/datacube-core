...
Minimal Example
~~~~~~~~~~~~~~~~
After importing the ``datacube`` package, users need to specify a name for their session, known as the ``app`` name. This name is generated by the user and is used to track down issues with database queries. It does not have any effect on the analysis. The resulting ``dc`` object provides access to all the data contained within the datacube instance.
