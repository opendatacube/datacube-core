...
- Removed product matching options from `datacube dataset update` (#445). No
...
- Usability improvements in `datacube dataset add|update` commands (#447, #448)
...
- Enable compression for metadata documents stored in NetCDFs generated by
  `stacker` and `ingestor` (#452)

- Better support stacked NetCDF files (#415)
...
- Allow specifying chunking for string variables when writing NetCDFs (#453) 
