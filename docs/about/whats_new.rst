...
- Removed product matching options from `datacube dataset update` (:pull:`445`). No
  matching is needed in this case as all datasets are already in the database and
  are associated to products.

- Removed `--match-rules` option from `datacube dataset add` (:pull:`447`)

- The seldom-used `stack` keyword argument has been removed from `Datcube.load`. (:pull:`461`)
...
- Usability improvements in `datacube dataset add|update` commands (:issue:`447`, :issue:`448`)
  - Embedded documentation updates
  - Deprecated `--auto-match` (it was always on anyway)
  - Renamed `--dtype` to `--product` (old version will still work, but with a warning)

- Enable compression for metadata documents stored in NetCDFs generated by
  `stacker` and `ingestor` (:issue:`452`)

- Better support stacked NetCDF files (:issue:`415`)
  - Record slice index as part of dataset location uri, using `#part=<int>` syntax, index is 0-based
  - Use this index when loading data instead of searching for timestamp
  - Fallback to old behaviour when `#part=<int>` is missing and file is more than one slice deep
...
- Fix a regression in v1.6rc1 whereby it was impossible to run ``datacube ingest``
  to create products which were defined in 1.5.5 and earlier versions of ODC. (:issue:`423`, :pull:`436`)

- Allow specifying chunking for string variables when writing NetCDFs (:issue:`453`) 
