...
def prepare_dataset(path):
    ...
    granuleslist = [(granule.get('granuleIdentifier'), [imid.text for imid in granule.findall('IMAGE_ID_2A')]) for granule in
    ...
    for granule_id, images in granules.items():
        ...
        scl_dict = {'SCL_20m': {'path': str(Path(classification)),'layer': 1, } for classification in images_classification}
...
@click.command(help="Prepare Sentinel 2 L2 sen2cor dataset SR and SC for ingestion into the Data Cube. eg. python sen2cor_prepare.py <input>.SAFE --output <outfile>.yaml")
