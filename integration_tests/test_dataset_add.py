...
def check_no_product_match(clirunner, index) -> None:
    ...
    r = clirunner(["dataset", "add", "--product", "A", str(prefix)])
    assert "ERROR" in r.output
    assert "Dataset metadata did not match product signature" in r.output

    r = clirunner(["dataset", "add", "--product", "A", "--product", "B", str(prefix)])
    assert "ERROR" in r.output
    ...
    r = clirunner(["dataset", "add", "--product", "B", "--ignore-lineage", str(prefix)])
...
def test_dataset_add(dataset_add_configs, index_empty, clirunner) -> None:
    ...
    r = clirunner(
        ["dataset", "info", "-f", "yaml", "--show-derived", str(ds.sources["ae"].id)]
    )
    assert str(ds.id) in r.output
    ...
    r = clirunner(
        ["dataset", "add", "--product", "nosuchproduct", p.datasets],
...
def test_dataset_add_stac(index, clirunner, ls8_stac_doc, eo3_products) -> None:
    ...
    assert err is None
    assert ds is not None
    assert ds.id == stac_doc.get("id")
    assert ds.product.name == "ga_ls8c_ard_3"

    r = clirunner(["dataset", "add", "--stac", path])
    assert r.exit_code == 0
...
def test_dataset_add_ambiguous_products(
    ...
    dss = [
        SimpleDocNav(dataset_maker(i)("A", product_type="eo", flag_a="a", flag_b="b"))
        for i in [1, 2]
    ]
...
def test_dataset_archive_dry_run(dataset_add_configs, index_empty, clirunner) -> None:
    ...
    valid_and_invalid_uuid = clirunner(
        ["dataset", "archive", "--dry-run", str(ds.id), non_existent_uuid],
        expect_success=False,
    )
...
def test_dataset_add_http(
    ...
    httpserver.expect_request("/metadata_types.yaml").respond_with_data(
        open(p.metadata).read()
    )
    httpserver.expect_request("/products.yaml").respond_with_data(
        open(p.products).read()
    )
    httpserver.expect_request("/datasets.yaml").respond_with_data(
        open(p.datasets).read()
    )
...
def xtest_dataset_add_fails(clirunner, index) -> None:
    ...
    assert result.exit_code != 0, (
        "Surely not being able to add a dataset when requested should return an error."
    )
