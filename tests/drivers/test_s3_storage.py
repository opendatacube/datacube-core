...
def test_create_s3lio(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    s = s3aio.S3LIO()


def test_chunk_indices_1d():
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3LIO()
    assert list(s.chunk_indices_1d(0, 10, 2)) == \
        [slice(0, 2, None), slice(2, 4, None), slice(4, 6, None), slice(6, 8, None), slice(8, 10, None)]
    assert list(s.chunk_indices_1d(0, 10, 2, slice(2, 8))) == [slice(2, 4, None), slice(4, 6, None), slice(6, 8, None)]
    assert list(s.chunk_indices_1d(0, 10, 2, slice(2, 8), True)) == [2, 2, 2]


def test_chunk_indices_nd():
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3LIO()
    x = np.arange(4*4*4, dtype=np.uint8).reshape((4, 4, 4))
    assert list(s.chunk_indices_nd(x.shape, (2, 2, 2))) == \
        [(slice(0, 2, None), slice(0, 2, None), slice(0, 2, None)), (slice(0, 2, None), slice(0, 2, None),
         slice(2, 4, None)), (slice(0, 2, None), slice(2, 4, None), slice(0, 2, None)), (slice(0, 2, None),
         slice(2, 4, None), slice(2, 4, None)), (slice(2, 4, None), slice(0, 2, None), slice(0, 2, None)),
         (slice(2, 4, None), slice(0, 2, None), slice(2, 4, None)), (slice(2, 4, None), slice(2, 4, None),
         slice(0, 2, None)), (slice(2, 4, None), slice(2, 4, None), slice(2, 4, None))]
    ...
    assert list(s.chunk_indices_nd(x.shape, (2, 2, 2), (slice(1, 3), slice(1, 3), slice(1, 3)))) == \
        [(slice(1, 2, None), slice(1, 2, None), slice(1, 2, None)), (slice(1, 2, None), slice(1, 2, None),
         slice(2, 3, None)), (slice(1, 2, None), slice(2, 3, None), slice(1, 2, None)), (slice(1, 2, None),
         slice(2, 3, None), slice(2, 3, None)), (slice(2, 3, None), slice(1, 2, None), slice(1, 2, None)),
         (slice(2, 3, None), slice(1, 2, None), slice(2, 3, None)), (slice(2, 3, None), slice(2, 3, None),
         slice(1, 2, None)), (slice(2, 3, None), slice(2, 3, None), slice(2, 3, None))]
...
def test_get_data_with_compression(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3LIO(True, False, str(tmpdir))
...
def test_get_data_without_compression(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3LIO(False, False, str(tmpdir))
...
def test_create_s3aio(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    s = s3aio.S3AIO()


def test_1d_nd_converions():
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3AIO()
...
def test_get_point_without_compression(tmpdir):
...
def test_get_point_with_compression(tmpdir):
...
def test_get_slice_with_compression(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
...
def test_get_slice_without_compression(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
...
def test_create_s3io(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    s = s3aio.S3IO(False)
    s = s3aio.S3IO(False, str(tmpdir))


def test_s3_resources(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    s = s3aio.S3IO(False, str(tmpdir))
    a = s.s3_resource()
...
def test_put_bytes_mpu_mp(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
    import numpy as np
    s = s3aio.S3IO(False, str(tmpdir))
    data = np.arange(20, dtype=np.uint8)
...
def test_put_bytes_mpu_mp_shm(tmpdir):
    import datacube.drivers.s3.storage.s3aio as s3aio
