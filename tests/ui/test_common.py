...
def test_get_metadata_path() -> None:
    ...
    assert (
        Path(path).absolute()
        == out_dir.joinpath("directory_dataset", "agdc-metadata.yaml.gz").absolute()
    )
    ...
    assert (
        Path(path).absolute()
        == out_dir.joinpath("file_dataset.tif.agdc-md.yaml").absolute()
    )
...
def test_ui_path_doc_stream(httpserver) -> None:
    ...
    for input_path, (doc, resolved_path) in zip(
        input_paths, ui_path_doc_stream(input_paths)
    ):
        assert doc == {}
