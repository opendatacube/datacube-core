...
def test_create_mask_value(simple_var) -> None:
    ...
    assert create_mask_value(bits_def, contiguous=True, land_sea="land") == (768, 768)
...
def test_create_multi_mask_value() -> None:
    ...
    assert create_mask_value(multi_flags_def, water_confidence="water") == (
        0b011000,
        0b011000,
    )

    assert create_mask_value(
        multi_flags_def, water_confidence="water", filled=True
    ) == (0b011001, 0b011001)
    assert create_mask_value(multi_flags_def, water_confidence="not_determined") == (
        0b011000,
        0b0,
    )
    assert create_mask_value(multi_flags_def, water_confidence="no_water") == (
        0b11000,
        0b01000,
    )
    ...
    assert create_mask_value(
        multi_flags_def, veg_confidence="maybe_veg", water_confidence="water"
    ) == (0b110011000, 0b100011000)
    assert create_mask_value(
        multi_flags_def,
        veg_confidence="maybe_veg",
        water_confidence="water",
        filled=True,
    ) == (0b110011001, 0b100011001)

    assert create_mask_value(multi_flags_def, water_confidence="maybe_water") == (
        0b011000,
        0b10000,
    )
...
@pytest.mark.parametrize(
    "bits_def", [SimpleVariableWithFlagsDef.flags_definition, bits_def_json]
)
...
def test_mask_valid_data() -> None:
    ...
    expected_data_array = DataArray(
        np.array(
            [[1.0, np.nan, np.nan], [2, 3, np.nan], [np.nan, np.nan, np.nan]],
            dtype="float",
        ),
        attrs=test_attrs,
        name="var_one",
    ...
    data_array = DataArray(
        [[1, -999, -999], [2, 3, -999], [-999, -999, -999]], attrs=test_attrs
    )
    dataset = Dataset(
        data_vars={"var_one": data_array}, attrs={"ds_attr": "still here"}
    )
...
def test_valid_data_mask() -> None:
    ...
    data_array = DataArray(
        [[1, -999, -999], [2, 3, -999], [-999, -999, -999]], attrs=attrs
    )
    ...
    data_array = DataArray(
        [[1, -999, -999], [2, 3, -999], [-999, -999, float("nan")]], attrs=attrs
    )
    dataset = Dataset(data_vars={"var_one": data_array})
