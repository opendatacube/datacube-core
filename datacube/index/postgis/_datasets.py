...
class DatasetResource(AbstractDatasetResource, IndexResourceAddIn):
    ...
    def get_unsafe(
        ...
        if include_deriveds:
            derived_tree = self._index.lineage.get_derived_tree(
                id_, max_depth=max_depth
            )
    ...
    def add(
        ...
        with self._db_connection(transaction=True) as transaction:
            ...
            is_new = transaction.insert_dataset(
                dataset.metadata_doc_without_lineage(), dataset.id, product_id
            )
            if is_new:
    ...
    def _add_batch(
        ...
        batch = BatchRep(
            datasets=[],
            uris=[],
            search_indexes={"string": [], "numeric": [], "datetime": []},
        ...
        for prod, metadata_doc, uri in batch_ds:
            ...
            for fname, finfo in search_field_vals.items():
                ...
                batch.search_indexes[search_key].append(
                    {"dataset_ref": dsid, "search_key": fname, "search_val": fval}
                )
        ...
        for q, product in product_queries:
            ...
            if return_fields:
                ...
                if select_field_names is None:
                    select_fields = tuple(
                        field
                        for name, field in dataset_fields.items()
        ...
        for q, product in product_queries:
            ...
            with self._db_connection() as connection:
                count = connection.count_datasets(
                    query_exprs, archived=archived, geom=geom
                )
            if count > 0:
                yield product, count
