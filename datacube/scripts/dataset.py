...
@ui.pass_index()
def update_cmd(index, keys_that_can_change, dry_run, location_policy, archive_less_mature, dataset_paths):
    if not dataset_paths:
        click.echo('Error: no datasets provided\n')
    ...
    for dataset, existing_ds in load_datasets_for_update(doc_stream, index):
        ...
        if not dry_run:
            try:
                index.datasets.update(dataset, updates_allowed=updates_allowed, archive_less_mature=archive_less_mature)
