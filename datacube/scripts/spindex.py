...
def create(index: Index, update: bool, srids: Sequence[str]) -> None:
    ...
    if confirmed:
        echo(
            f"Spatial indexes created for: {','.join(str(crs.epsg) for crs in confirmed)}"
        )
...
@system.command(
    "list", help="List all CRSs for which spatial indexes exist in this index"
)
@ui.pass_index()
...
def drop(index: Index, force: bool, srids: Sequence[str]) -> None:
    ...
    if for_deletion and not force:
        echo(
            "WARNING: Recreating spatial indexes may be slow and expensive for large databases."
        )
        echo(
            "You have requested to delete spatial indexes for the following "
            f"CRSes: {','.join(str(crs.epsg) for crs in for_deletion)}"
        )
        if sys.stdin.isatty():
            confirmed = confirm(
                "Are you sure you want to delete these spatial indexes?", default=False
            )
            if not confirmed:
