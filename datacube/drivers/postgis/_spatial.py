...
from ._schema import orm_registry, Dataset, SpatialIndexRecord

_LOG = logging.getLogger(__name__)


# Base class for SpatialIndex ORM models
class SpatialIndex:
    ...
...
class SpatialIndexORMRegistry:
    ...
    def _mint_new_spindex(self, epsg: int):
        ...
        # Add geometry column
        attributes["extent"] = Column(Geometry('MULTIPOLYGON', srid=epsg), nullable=False, comment="The extent of the dataset")
...
def spindex_for_record(rec: SpatialIndexRecord) -> Type[SpatialIndex]:
    sir = SpatialIndexORMRegistry()
    return sir.get(rec.crs)


def ensure_spindex(engine: Connectable, sp_idx: Type[SpatialIndex]) -> Type[SpatialIndex]:
